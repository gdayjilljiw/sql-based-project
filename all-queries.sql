...
--- top spenders
with sum_sales_customers as ( 
	select customer_id, round(cast(sum(sales) as numeric), 2) total 
	from order_sales_overview
	group by customer_id ),
	customer_ranks as (
	select sc.customer_id, c.customer_name, sc.total, rank() over(order by total desc )
	from sum_sales_customers sc
...
--- RFM
--- recency freqeuncy monetary
--- most recent customers
select customer_id, max(order_date) most_recent_purchase,
ntile(5) over(order by max(order_date)) recency_score
from order_sales_overview
group by customer_id 
...
with rfm_scores as(
	...
	count(customer_segment) over(partition by customer_segment)
	from segments 
